...
c(3>0,-3<0) && c(3%in%1:5, -3%in%1:5) # Compare the first elements of two, ignores the second
c(-3>0,3<0) && c(-3%in%1:5, 3%in%1:5)
...
# Basics of matrices
# Matrix is a two-dimmenstional array of numbes and strings
...
ncol(mymat2) # the number of columns
length(mymat2) # the number of elements

# Define the names of columns and rows
rownames(mymat) <- LETTERS[1:3]
colnames(mymat) <- letters[1:10]
mymat

# submatrix selection
mymat1[c(1,3), ] # first component is for rows you want to pick and second is for columns, if blank, you choose all
mymat1[2:3,c(3,5)]
...
# Matrix Operations
trans_mymat <- t(mymat) # t() transposes
ele_pro <- mymat1 * mymat # hadamard (entrywise) multiplication
mat_pro <- t(mymat1) %*% mymat
matpro2 <- crossprod(mymat1, mymat) #since the second argument is the same
mymat / mymat1 # elementwise
# crossprod(A) = t(A)%*%B

sum(mat_pro) # sum of all elements
